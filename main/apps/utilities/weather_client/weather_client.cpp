...
namespace WEATHER_CLIENT
{
    ...
    WeatherInfo get_weather(const char* base_url)
    {
        ...
        if (cJSON_IsString(city)) result.city = city->valuestring;

        cJSON* feels = cJSON_GetObjectItem(root, "feels_like_c");
        if (cJSON_IsString(feels)) result.feels_like_c = feels->valuestring;

        cJSON* humidity = cJSON_GetObjectItem(root, "humidity");
        if (cJSON_IsString(humidity)) result.humidity = humidity->valuestring;

        cJSON* code = cJSON_GetObjectItem(root, "condition_code");
        if (cJSON_IsString(code)) result.condition_code = code->valuestring;
